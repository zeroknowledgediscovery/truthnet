...
class truthnet:
    ...
    def fit(self, alpha=0.1, shap_index=None, shapnum=10, nullsteps=100000, veritas_version='0.0.1'):
        ...
        Xpos_training = df_training_pos.drop(self.target_label, axis=1).values.astype(str)
        Xneg_training = df_training_neg.drop(self.target_label, axis=1).values.astype(str)
        featurenames = df_training_pos.drop(self.target_label, axis=1).columns
        ...
        X = df_test.drop(self.target_label, axis=1).values.astype(str)
    ...
    def calibrate(self, qsteps=1000, calibration_num=10000):
        ...
        for i in tqdm(range(calibration_num)):
            ...
            ff = pd.DataFrame(sq.reshape(1, -1), columns=featurenames)[featurenames[self.shap_index[:self.query_limit]]]
            adict = np.append(adict, {'xx' + str(i): ff.iloc[0].to_dict()})
        
        Rjson = reveal(list(adict), self.veritas_model, perturb=0, ci=False, model_path=False)
        r_ = [(x.get('veritas'), x.get('score'), x.get('lower_threshold')) for x in Rjson[0]]
