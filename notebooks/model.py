...
class model:
    ...
    def fit(
        ...
        if num_None == 0:
            raise ValueError(
                "input either samples and features or data object, not both!"
            )
        elif data_obj is not None:
            (
                featurenames,
                samples,
            ) = data_obj.Qnet_formatter()  # returns the training data
            print(len(samples))
            self.immutable_vars, self.mutable_vars = (
                data_obj.immutable_vars,
                data_obj.mutable_vars,
            )
        elif num_None > 1:
    ...
    def save(self, file_path=None, low_mem=False):
        ...
        """
        assert_None([self.qnet])
        if file_path is None:
            file_path = "tmp_Qnet.joblib"
        save_qnet(self.qnet, file_path, low_mem=low_mem)

    def load(self, file_path, gz=False, VERBOSE=False):
        """load Qnet from file
        ...
        if "https" in file_path or "www." in file_path:
            ...
            self.qnet = load_qnet(url_file, gz=gz)
        else:
            self.qnet = load_qnet(file_path, gz=gz)
        self.features = self.qnet.feature_names
        if VERBOSE:
            print("done")
        return self.qnet
