# %%
from truthfinder import truthfinder

# %% Global
problem_type = "global"
maling_type = "runif"
n_questions = 346
model_path = f"models/{problem_type}/random_order_full_model_0.joblib.gz"
classifier_path = (
    f"classifiers/{problem_type}/{maling_type}-classifier-{n_questions}.pkl"
)
roc_path = f"classifiers/{problem_type}/{maling_type}-roc-{n_questions}.pkl"

patients_responses = [
    {
        "0": {
            "20": "1",
            "31": 2.0,
            "36": 1.0,
            "51": 2.0,
            "60": 1.0,
            "75": 1.0,
            "85": 1.0,
            "115": 1.0,
            "123": 2.0,
            "147": 2.0,
            "150": 3.0,
            "179": 2.0,
            "312": 2.0,
            "355": 2.0,
            "436": 4.0,
            "572": 3.0,
            "719": 2.0,
            "737": 2.0,
            "747": 2.0,
            "817": 2.0,
            "855": 2.0,
            "938": 1.0,
            "983": 2.0,
            "1053": 2.0,
            "1060": 2.0,
            "1069": 1.0,
            "1132": 2.0,
            "4201": 1.0,
            "4203": 1,
            "4247": 1.0,
            "4250": 2.0,
            "4251": 2.0,
            "4254": 2.0,
            "4255": 1,
            "4262": 2.0,
            "4278": 1,
            "4280": 1,
            "4287": 1,
            "4289": 1,
            "4439": 1.0,
            "4441": 1.0,
            "4442": 2.0,
            "4452": 1.0,
            "4453": 0,
            "4454": 0,
            "4455": 0,
            "4456": 0.0,
            "4525": 1,
            "4527": 1,
            "4529": 3.0,
            "4531": 1.0,
            "4532": 2.0,
            "4545": 3.0,
            "4553": 1.0,
            "4555": 1,
            "4571": 1.0,
            "4574": 2.0,
            "4587": 1.0,
        }
    },
    {
        "9": {
            "20": 2.0,
            "31": 1.0,
            "36": 1.0,
            "46": 1.0,
            "51": 1.0,
            "53": 1.0,
            "61": 2.0,
            "62": 2.0,
            "76": 1.0,
            "85": 1.0,
            "109": 3.0,
            "110": 4.0,
            "111": 3.0,
            "123": 2.0,
            "149": 4.0,
            "210": 3.0,
            "255": 2.0,
            "576": 1.0,
            "603": 4.0,
            "840": 2.0,
            "841": 2.0,
            "855": 2.0,
            "881": 2.0,
            "927": 2.0,
            "928": 2.0,
            "936": 2.0,
            "1009": 3.0,
            "1041": 3.0,
            "1080": 3.0,
            "1125": 2.0,
            "4203": 1,
            "4247": 1.0,
            "4248": 4.0,
            "4250": 2.0,
            "4251": 3.0,
            "4255": 3,
            "4262": 1.0,
            "4278": 1,
            "4280": 1,
            "4287": 1,
            "4289": 1,
            "4355": 3.0,
            "4439": 3.0,
            "4441": 3.0,
            "4451": 2.0,
            "4452": 3.0,
            "4453": 0,
            "4454": 0,
            "4455": 0,
            "4456": 0.0,
            "4525": 2,
            "4527": 1,
            "4554": 2.0,
            "4555": 3,
            "4586": 1.0,
            "4587": 3.0,
            "4590": 1.0,
            "4596": 3.0,
        }
    },
    {
        "PHX1": {
            "20": "1",
            "31": 2.0,
            "36": 1.0,
            "51": 2.0,
            "60": 1.0,
            "75": 1.0,
            "85": 1.0,
            "115": 1.0,
            "123": 2.0,
            "147": 2.0,
            "150": 3.0,
            "179": 2.0,
            "312": 2.0,
            "355": 2.0,
            "436": 4.0,
            "572": 3.0,
            "719": 2.0,
            "737": 2.0,
            "747": 2.0,
            "817": 2.0,
            "855": 2.0,
            "938": 1.0,
            "983": 2.0,
            "1053": 2.0,
            "1060": 2.0,
            "1069": 1.0,
            "1132": 2.0,
            "4201": 1.0,
            "4203": 1,
            "4247": 1.0,
            "4250": 2.0,
            "4251": 2.0,
            "4254": 2.0,
            "4255": 1,
            "4262": 2.0,
            "4278": 1,
            "4280": 1,
        }
    },
]


result = truthfinder(patients_responses, model_path, classifier_path, roc_path)
print(result)

# %% PTSD

problem_type = "ptsd"
maling_type = "runif"
n_questions = 211
model_path = f"models/{problem_type}/random_order_full_model_0.joblib.gz"
classifier_path = (
    f"classifiers/{problem_type}/{maling_type}-classifier-{n_questions}.pkl"
)
roc_path = f"classifiers/{problem_type}/{maling_type}-roc-{n_questions}.pkl"

import pandas as pd

patients_responses = pd.read_csv("../data/ptsd/PTSD_cognet_test_processed.csv").to_dict(
    "index"
)
processed_responses = [
    {patient: patients_responses[patient]} for patient in patients_responses
]
result = truthfinder(processed_responses, model_path, classifier_path, roc_path)
print(f"PTSD {maling_type} results:\n", result)


# %% CCHHS

problem_type = "bond-court"
maling_type = "expert"
n_questions = 42
model_path = f"models/{problem_type}/random_order_full_model_0.joblib.gz"
classifier_path = (
    f"classifiers/{problem_type}/{maling_type}-classifier-{n_questions}.pkl"
)
roc_path = f"classifiers/{problem_type}/{maling_type}-roc-{n_questions}.pkl"

import pandas as pd

patients_responses = (
    pd.read_csv("../data/bondcourt/CCHHS_SUD_details.csv")
    .drop("Interview_ID", axis="columns")
    .fillna(-9)
    .astype(int)
    .replace(-9, "")
    .to_dict("index")
)
processed_responses = []
for patient in patients_responses:
    dict = patients_responses[patient].copy()
    for k in list(dict):
        if dict[k] == "":
            del dict[k]
    processed_responses.append({patient: dict})

result = truthfinder(processed_responses, model_path, classifier_path, roc_path)

print("CCHHS {maling_type} results:\n", result)

# %%
