...
\begin{table}[t]
  ...
  \captionN{Prolific Dataset  Average Characteristics and Success Rates as a Percentage of Representation, at 90\% sensitivity}\label{tabprolific}
...
\begin{table}[t]
  ...
  \captionN{Minimum performance trade-offs (PPV and NPV are calculated assuming prevalence of 30\%)}\label{tabperf}


  \sffamily\fontsize{10}{8}\selectfont
  
\input{Figures/tabperf}
\end{table}
...
%malhotra2008completion,callegaro2009response}, demonstrating a lack of attention by interview respondents\cite{greszki2015exploring}.
...
%\vrts moves  away from the idea of detecting malingering  by employing detection strategies focused on symptoms that are either unlikely to be presented by genuine patients, or that tend to be amplified in malingerers~\cite{rogers2008determinations}, or other  physiological or  arousal-based approaches such as the Control Question Technique\cite{vrij2001killed}, or cognitive load-inducing approaches such as requiring subjects to perform a concurrent task ($e.g.$ maintaining eye contact) and the Autobiographical Implicit Association Test (aIAT).

The motivation behind fabricating PTSD symptoms is multifaceted, often driven by financial incentives such as disability compensation, insurance claims, or legal benefits. % This might be  prevalent among veterans, who may receive significant compensation if diagnosed with PTSD linked to military service, and among individuals in the criminal justice system, where fabricating PTSD may be used to mitigate legal responsibilities or penalties.
